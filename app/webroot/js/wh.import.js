function SetExportValue(item, value){
	$.post('?mod=import&site=ajax_set_export_value',{'item': item, 'value': value}).done(function(data){
...
function AddProduct(id){
	...
	$.post('?mod=import&site=ajax_add_product_session',{'id': id})
	.done(function(data){
		if(data=='0'){
			return false;
		}
...
function UpdateNumberProduct(id, type, number){
	...
	$.post('?mod=import&site=ajax_update_product_number',{'id': id, 'type':type, 'number': number})
	.done(function(data){
...
function UpdateProductPrice(id, price){
	$.post('?mod=import&site=ajax_change_product_price',
			{'id':id, 'price':price}
	).done(function(data){
		var data = JSON.parse(data);
...
function LoadDataForFormUpdatePrice(id) {
	...
	$.post("?mod=product&site=ajax_load_price_item", {'id' : id}).done(function(data) {
		var data = JSON.parse(data);
		console.log(data);
		$("#UpdateExportPrice input[name=id]").val(data.product_id);
...
function LoadProduct(){
	...
	$.post('?mod=import&site=ajax_load_product',post)
	.done(function(data){
...
function Refresh(){
	$.post('?mod=import&site=ajax_refresh')
	.done(function(rt){
		LoadProduct();
		$("#Products tbody").html('');
		GetTotalSession();
...
function GetTotalSession(is_payment){
	$.post('?mod=import&site=ajax_get_total_session', {'is_payment':is_payment}).done(function(data){
		var data = JSON.parse(data);
