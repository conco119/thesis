...
<div class="container"  id="printA4">
	...
	<div class="row">
	    {if $arg.setting.use_logo eq 1}
	       <div class="col-md-2 col-sm-2 col-xs-2"><img src="{$output.logo}" style="max-width: 100px; position: absolute; left: 0px; top: 10px;" /></div>
	    {/if}
	   <div class="col-md-5 col-sm-5 col-xs-5">
	    ...
	    <div class="col-md-5 col-sm-5 col-xs-5">
	            <p>Nhân viên nhập: {$value.creator} <span class="pull-right p">Mã phiếu: [{$out.code}]</span></p>
	            <p>Nhà cung cấp: {$out.supplier} <span class="pull-right p">Ngày Tháng: {$value.date}</span></p>
	    </div>
	</div>
    ...
    <table class="table table-bordered mar-top">
            <thead>
                <tr>
                    ...
                    {if $arg.setting.percent eq 1} <th class="">Nguyên giá</th>
                     <th class="">Chiết khấu</th>{/if}
