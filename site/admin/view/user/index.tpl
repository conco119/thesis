    					...
    					<table class="table table-striped table-hover projects">
    						...
    						<tbody>
    							...
    							<tr id="field{$data.id}">
    								...
    								<td><a href="#">{$data.username} ({$data.name})</a> <br /> <small>Created {$data.created}</small></td>
    								<td>{$data.group_name}</td>
    								<td class="text-center" id="stt{$data.id}">
    									{$data.status}
    								</td>
    								<td class="text-center"><input type="number" name="sort" onchange="sortItem('users', {$data.id}, this.value);" value="{$data.sort}"></td>
    								<td class="text-right">
...
function activeUser(table, id) {
    $.post("?mod=user&site=ajax_active_user", {'table': table, 'id': id}).done(function (data) {
        if (data == '0')
            alert('khong the cap nhat trang thai');
        else if (data == '2')
            alert('Khong co quyen thay doi');
        else
            $("#stt" + id).html(data);
    });
}
